 ...
 * 說明：基本插入排序在最壞情況下，每插入一個元素都需與前面所有元素比較並移動，
 *       因此總共最多需比較約 n(n-1)/2 次。最佳情況是資料已排序，僅需 O(n)。
 */

/* 
 * Time Complexity: O(n^2)
 * 說明：二分插入排序雖然使用 binary search 降低了插入點查找成本至 O(log n)，
 *       但仍需 O(n) 時間將元素右移，因此總時間為 O(n log n)（查找） + O(n^2)（移動），
 *       主因仍為移動成本，故總體仍為 O(n^2)。
 */
